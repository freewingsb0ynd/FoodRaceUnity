...
public class GameManager : MonoBehaviour
{
    ...
    public int candyShowAllCount;
    public Text candyShowAllText;
    public int candyFreezeTimeCount;
    public Text candyFreezeTimeText;
    ...
    private float stopTimeLeft = 5.0f;

    private bool revealingAll = false;
    private float revealTimeLeft = 2.0f;
    ...
    void Update()
    {
        if (play == true)
        {
            ...
            int isSetting = PlayerPrefs.GetInt("isSetting", 0);
            if (isSetting != 1)
            {
                if (!stopTime)
                {
                ...
                {
                    stopTime = false;
                    PlayPanel.GetComponent<Image>().color = new Color(1.0f, 1.0f, 1.0f, 0.0f);
                    stopTimeLeft = 5f;
                }
                if (revealingAll) revealTimeLeft -= Time.deltaTime;
    ...
    public void CandyShowAll()
    {
        if (candyShowAllCount > 0)
        {            
            revealingAll = true;
            for (int i = 0; i < cards.Length; i++)
            {
            ...
            candyShowAllCount--;
            candyShowAllText.text = " x " + candyShowAllCount;
        }
        if (candyShowAllCount == 0)
        {
            Debug.Log("No Show All Candy");
        }
    }

    public void CandyFreezeTime()
    {
        if (candyFreezeTimeCount > 0)
        {
            stopTime = true;
            PlayPanel.GetComponent<Image>().color = new Color(0.5f, 0.5f, 0.5f, 0.4f);
            
            candyFreezeTimeCount--;
            candyFreezeTimeText.text = " x " + candyFreezeTimeCount;
        }
        if (candyFreezeTimeCount == 0)
        {
            Debug.Log("No Freeze Time Candy");
        }
    }
    public void Back()
    {
    ...
    public void Restart()
    {
        TKSceneManager.ChangeScene(levelName);
        score = 0;
    }
    ...
    public void initializeCardsFirstTime()
    {
        if (!_init) {
